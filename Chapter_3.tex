...
The remaining part of this communication are arranged as follows. Section \ref{sec-prop-alg} contains the details of the proposed method for implementing the genetic algorithm in AEDT. Section \ref{sec-exp-details} covers the details of the experiment formulated to test the proposed approach. The experimental results are discussed in Section \ref{expt-res}. Finally, the description is concluded in Section \ref{concl}.

\section{Details of the Proposed Algorithm} \label{sec-prop-alg}
\begin{figure}
  \centering
...
This section covers the details of an experimental evaluation of genetic algorithm for optimizing a printed antenna. The method discussed in the Section \ref{sec-prop-alg} is used here to optimize an H-shaped slot antenna. Slot antennas are printed antennas. Unlike microstrip antennas, the slot antenna has a slotted section etched from the ground plane. Slot antennas are excited with a microstrip line present at the opposite face of the PCB. Coplanar wave-guide (CPW) may also be used for exciting a slot antenna.
...
\section{Experimental Results} \label{expt-res}
